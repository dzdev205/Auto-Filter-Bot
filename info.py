...
# Bot information
API_ID = environ.get('API_ID', '39047562')  #api id of your telegram id
if len(API_ID) == 0:
    print('Error - API_ID is missing, exiting now')
    exit()
else:
    API_ID = int(API_ID)
API_HASH = environ.get('API_HASH', '1804377b8fc71ff1d2f7dd70c4b16dca') #api hash of your telegram id
if len(API_HASH) == 0:
    print('Error - API_HASH is missing, exiting now')
    exit()
BOT_TOKEN = environ.get('BOT_TOKEN', '8534559391:AAEjwB1lvypAR6UZ6VeWD5fv8-sIfOH1J7s') #bot token from botfather
if len(BOT_TOKEN) == 0:
...
# Bot Admins
ADMINS = environ.get('ADMINS', '6982780129 7310522968') #apni tg id daalo
if len(ADMINS) == 0:
...
# Channels
INDEX_CHANNELS = [int(index_channels) if index_channels.startswith("-") else index_channels for index_channels in environ.get('INDEX_CHANNELS', '8534559391').split()]
if len(INDEX_CHANNELS) == 0:
    print('Info - INDEX_CHANNELS is empty')
AUTH_CHANNEL = [int(auth_channels) for auth_channels in environ.get('AUTH_CHANNEL', '-5010406746').split()]
if len(AUTH_CHANNEL) == 0:
    print('Info - AUTH_CHANNEL is empty')
LOG_CHANNEL = environ.get('LOG_CHANNEL', '-5091311014') #bot log channel -1005293546253
...
# support group
SUPPORT_GROUP = environ.get('SUPPORT_GROUP', '-5091311014') #support group id ex:  -1002936246860
...
# MongoDB information
DATABASE_URL = environ.get('DATABASE_URL', "mongodb+srv://houaidjihoussem_db_user:41eOz8Cuy2OiGBy3@cluster0.nj3eywa.mongodb.net/?appName=Cluster0") #mongo db url
...
BIN_CHANNEL = environ.get("BIN_CHANNEL", "-1003228882578") #if is_stream = true then add a channel id ex: -10026393639
if len(BIN_CHANNEL) == 0:
    print('Error - BIN_CHANNEL is missing, exiting now')
    exit()
else:
    BIN_CHANNEL = int(BIN_CHANNEL)
URL = environ.get("URL", "https://shiny-centipede-autofilte-e0f166c3.koyeb.app/") #if heroku then paste the app link here ex: https://heroku......./
if len(URL) == 0:
